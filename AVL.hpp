...
void split(int x, BT &S1, BT &emptyTree){
    ...
    if(parent1 != S1.root && object != S1.root){
        ...
        std::cout << parent1parent->key << std::endl;
        std::cout << parent1->key << std::endl;

        if(object->key <= parent1->key && parent1->key <= parent1parent->key){
        ...
        if(object->key > parent1->key && parent1->key > parent1parent->key){ //OK
